...
// import net.dima.dima5_project.handler.LoginFailureHandler;
// import net.dima.dima5_project.handler.LoginSuccessHandler;
...
//     private final CustomLogoutSuccessHandler logoutSuccessHandler; // LogoutSuccessHandler은 이미 있는 객체이므로 커스텀
...
//                 "/js/**", 
//                 "/css/**").permitAll()                         // 모든 사람들에게 주어지는 경로
...
//             http
//                 .formLogin((auth) -> auth
//                     .loginPage("/user/login")                        // 로그인 없이 특정페이지에 접속을 시도하면 무조건 로그인 페이지로 이동 = redirect
//                     .loginProcessingUrl("/user/loginProc")  // 로그인 화면에서 로그인 버튼을 클릭하면 시큐리티가 받아서 처리(컨트롤러에서는 처리하지 않음)
...
//                     //.failureUrl("/login?error=true")                         // 로그인 에러가 나면 다시 로그인 페이지로 이동, 에러를 파라미터로 설정  
//                     .permitAll()
//                     );
